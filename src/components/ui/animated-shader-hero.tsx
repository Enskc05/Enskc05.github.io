...
class WebGLRenderer {
  ...
  private mouseMove = [0, 0] as [number, number]
  private mouseCoords = [0, 0] as [number, number]
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		vec2 p=uv;
		float d=length(p);
		// Changed star colors from warm tones to cool cyan/blue tones
		col+=.00125/d*(cos(sin(i)*vec3(0.5,1.2,1.8))+0.5);
		float b=noise(i+p+bg*1.731);
		col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
		// Smooth slate/cyan tones matching background - removed orange/red
		col=mix(col,vec3(bg*.05,bg*.12,bg*.18),d*0.8);
	}
	// Smooth out any remaining warm tones - reduce red, enhance blue
	col.r*=0.7;
	col.b*=1.2;
	O=vec4(col,1);
}`
...
const useShaderBackground = () => {
  ...
  const resize = useCallback(() => {
    ...
    resizeTimeoutRef.current = setTimeout(() => {
      ...
      // Always use window dimensions for better performance
...
const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-black ${className}`}>
      ...
      {(headline.line1 || headline.line2 || subtitle || buttons) && (
